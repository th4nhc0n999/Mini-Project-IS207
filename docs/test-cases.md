...
| ID | Test Case | Dữ liệu đầu vào | Kết quả mong đợi | HTTP Status |
|---|---|---|---|---|
| **DOC-01** | Lấy danh sách chuyên khoa | Không yêu cầu auth | Trả về danh sách chuyên khoa (id, name, slug, image_url) | `200 OK` |
...
| ID | Test Case | Dữ liệu đầu vào | Kết quả mong đợi | HTTP Status |
|---|---|---|---|---|
| **BOOK-01** | Đặt lịch thành công khi slot còn chỗ | `doctor_id`, `slot_id` hợp lệ, slot chưa đầy | Tạo booking thành công, tăng `booked_count` của slot, sinh `booking_code` | `201 Created` |
| **BOOK-02** | Đặt lịch khi slot đã đầy | Slot có `booked_count >= max_patients` | Ném `SlotUnavailableException`, thông báo slot đã hết chỗ | `422 Unprocessable` |
...
| **BOOK-04** | Kiểm thử đồng thời (Concurrency Test) | 2 yêu cầu đặt cùng lúc cho slot chỉ còn 1 chỗ trống duy nhất | Database Transaction + `lockForUpdate`: 1 request thành công (201), 1 request bị từ chối với lỗi slot đã đầy (422) | `201` & `422` |
...
| ID | Test Case | Dữ liệu đầu vào | Kết quả mong đợi | HTTP Status |
|---|---|---|---|---|
| **ADM-01** | User với role `patient` truy cập API Admin | Header kèm token của bệnh nhân | `CheckRole` middleware chặn lại, trả về lỗi Không có quyền | `403 Forbidden` |
| **ADM-02** | Admin tạo mới chuyên khoa | Dữ liệu chuyên khoa hợp lệ | Tạo chuyên khoa thành công | `201 Created` |
| **ADM-03** | Admin tạo mới slot ca khám | `doctor_id`, `date`, `start_time`, `end_time`, `max_patients` | Slot được tạo ở trạng thái `available` | `201 Created` |
